...
def test_mcp_server(base_url="http://localhost:8000"):
    ...
    try:
        ...
        print("   ❌ Cannot connect to MCP server - is it running with Streamable HTTP transport?")
        return False
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            if "result" in data:
                ...
                print(f"   ⚠️  Tool call error (expected if no API key): {data['error'].get('message', 'Unknown error')}")
            else:
    ...
    print(f"🌐 Protocol: JSON-RPC over HTTP")
    return True
...
if __name__ == "__main__":
    ...
    parser.add_argument("--quick", action="store_true", 
                       help="Run quick tests only (same as default)")
