...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8000/mcp-test", help="MCP server URL"
    )
            ...
            if args.command == "list-tools":
                ...
                try:
                    response = await call_tool(
                        client, base_url, args.tool_name, arguments
                    )
                    print("Status: 200")
                    print(json.dumps(response, indent=2))
                except Exception as e:
                ...
                status, response = await read_mcp_resource(
                    client, base_url, "splitwise://balance"
                )
                print(f"   Status: {status}")
...
if __name__ == "__main__":
    ...
    if not os.getenv("SPLITWISE_API_KEY") and not os.getenv("SPLITWISE_CONSUMER_KEY"):
        print(
            "Warning: No Splitwise API credentials found. Some tests may fail.",
            file=sys.stderr,
        )
        print(
            "Set SPLITWISE_API_KEY or SPLITWISE_CONSUMER_KEY/SPLITWISE_CONSUMER_SECRET",
            file=sys.stderr,
        )
