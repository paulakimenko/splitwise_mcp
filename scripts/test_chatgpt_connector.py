...
def test_list_tools(base_url: str, session_id: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(
            base_url, json=list_tools_request, headers=headers, timeout=10
        )
...
def main():
    ...
    print(f"\n{'=' * 80}")
    print("ChatGPT MCP Connector Test")
    print(f"{'=' * 80}")
    print("This simulates what ChatGPT does when connecting to an MCP server.")
    ...
    if "result" not in response:
        ...
        print("ChatGPT expects a standard JSON-RPC response with 'result' field")
        sys.exit(1)
    ...
    print("✅ MCP server is compatible with ChatGPT connectors")
