...
def test_initialize(url: str) -> tuple[bool, dict[str, Any] | None, str | None]:
    ...
    try:
        ...
        print_result(True, f"Status code: 200 OK")
        ...
        if "tools" not in capabilities:
            print_result(False, "❌ CRITICAL: Missing 'tools' capability - ChatGPT won't show connector!")
            return False, None, None
        ...
        if not server_info:
            ...
            return False, None, None
        print_result(True, f"Server: {server_info.get('name')} v{server_info.get('version')}")
...
def test_tools_list(url: str, session_id: str | None) -> tuple[bool, list[dict[str, Any]] | None]:
    ...
    try:
        ...
        if len(tools) == 0:
            print_result(False, "❌ CRITICAL: Empty tools array - ChatGPT won't show connector!")
            return False, None
        ...
        for i, tool in enumerate(tools):
            ...
            if missing_fields:
                print_result(False, f"  {tool_name}: Missing fields: {', '.join(missing_fields)}")
                all_valid = False
            else:
                ...
                if not isinstance(input_schema, dict):
                    ...
                    print_result(False, f"  {tool_name}: inputSchema type must be 'object'")
                    all_valid = False
                else:
...
def print_summary(init_passed: bool, tools_passed: bool, tools_count: int) -> None:
    ...
    if init_passed and tools_passed and tools_count > 0:
        ...
        print("1. Developer Mode is enabled: Settings → Connectors → Advanced → Developer mode")
        print("2. Refresh the connector: Settings → Connectors → Click connector → Refresh")
...
def main() -> None:
    ...
    if len(sys.argv) != 2:
        ...
        print("Example: python scripts/verify_mcp_compliance.py https://sw-mcp.paulakimenko.xyz/mcp")
        sys.exit(1)
