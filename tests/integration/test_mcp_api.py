...
class TestMCPIntegration:
    ...
    def test_list_groups_endpoint(self, test_client: TestClient):
    ...
    def test_get_current_user_endpoint(self, test_client: TestClient):
    ...
    def test_get_groups_endpoint(self, test_client: TestClient):
        """Test getting groups via MCP endpoint."""
        response = test_client.post(
            "/mcp/list_groups", json={}
        )  # Use list_groups instead of get_groups

        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)

    def test_list_expenses_endpoint(self, test_client: TestClient):
    ...
    def test_list_friends_endpoint(self, test_client: TestClient):
        """Test listing friends via MCP endpoint."""
        response = test_client.post("/mcp/list_friends", json={})

        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)

    def test_unsupported_method(self, test_client: TestClient):
...
class TestMCPWithTestGroup:
    ...
    def test_get_group_by_name(
        ...
        response = test_client.post(
            "/mcp/get_group", json={"args": {"id": test_group_id}}
        )
    ...
    def test_get_group_expenses(self, test_client: TestClient, test_group_id: int):
        """Test getting expenses for a specific group."""
        response = test_client.post(
            "/mcp/list_expenses", json={"args": {"group_id": test_group_id}}
        )
